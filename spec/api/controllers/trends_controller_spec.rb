...
RSpec.describe Api::TrendsController, type: :controller do
  ...
  let(:trend) { create(:trend, user_id: user.id) }
  ...
  context "Authenticated user" do
    ...
    describe "POST create" do
      before { post :create, params: { trend: { name: trend.name, user_id: trend.user_id } } }
      ...
      it "creates a trend with the correct attributes" do
        ...
        expect(hashed_json["name"]).to eq(trend.name)
      end
    end

    describe "DELETE destroy" do
      before { delete :destroy, id: trend.id }
